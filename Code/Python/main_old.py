...
def main():
    ...
    while (diff > tol):
        ...
        rhs = C1.dot(u / dt - convective - C2.dot(u / Re) - C3)

        P = np.linalg.solve(A, rhs)

        uOld = np.copy(u)

        u -= dt * (convective + E10.dot(P) + C2.dot(u / Re) + C3)
    ...
    np.savetxt("u.dat", u, delimiter='\t', fmt='%f')
...
def generateConvective(N, xi, u, uK, h):
    ...
    for i in range(1, N):
        for j in range(0, N):
            ...
            V1 = v[i + j * (N + 2)] + v[(i + 1) + j * (N + 2)]
            V2 = v[i + (j + 1) * (N + 2)] + v[(i + 1) + (j + 1) * (N + 2)]

            convective[k] = - h[i] / (4 * h[j]) * V1 * xi[i+j*(N+1)] - h[i] / (4 * h[j+1]) * V2 * xi[i+(j+1)*(N+1)]

    for i in range(0, N):
        for j in range(1, N):
            k = N * (N - 1) + i + N * (j - 1)

            U1 = u[i + j * (N + 1)] + u[i + (j + 1) * (N + 1)]
            U2 = u[(i + 1) + j * (N + 1)] + u[(i + 1) + (j + 1) * (N + 1)]

            convective[k] = h[j] / (4 * h[i]) * U1 * xi[i+j*(N+1)] + h[j] / (4 * h[i+1]) * U2 * xi[(i+1)+j*(N+1)]
