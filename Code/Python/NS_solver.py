...
if __name__ == '__main__':
    Re = np.arange(500, 10500, 500)
    N = np.arange(4, 21, 4)
    
    print(Re)
    print(N)
        
    results = np.zeros((np.size(N), np.size(Re)))
        
    for i in np.arange(np.size(N)):
        for j in np.arange(np.size(Re)):
            print('N = ', N[i], ', Re = ', Re[j])
            
            result = main(1e-5, Re[j], 0.005, N[i])
