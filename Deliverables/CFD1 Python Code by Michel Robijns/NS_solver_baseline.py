...
def main():
    ...
    while (diff > tol):
        ...
        rhs = tE21.dot(Ht11).dot(u / dt - 
              H1t1.dot(tE10).dot(Ht02).dot(E21).dot(u / Re) - uPres / Re
              - convective)
        ...
        u = u - dt * (E10.dot(P) + H1t1.dot(tE10).dot(Ht02).dot(E21).dot(u /
            Re) + uPres / Re + convective)

        # Check for convergence and check the rate of mass creation (must be
        # in the order of machine precision)
        if (iteration % 100 == 0):
            maxdiv = max(tE21.dot(Ht11).dot(u))
